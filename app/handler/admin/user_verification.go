...
import (
	...
	"donkey-ucenter/app/service/iuser_verification/user_verification_admin"
...
func (hdl *userVerificationHdl) Query(c *gin.Context) {
	form := new(user_verification_def.UserVerificationQueryForm)
	...
	ret, err := user_verification_admin.AdminSrv.Query(form)
...
func (hdl *userVerificationHdl) GetList(c *gin.Context) {
	...
	ret, err := user_verification_admin.AdminSrv.GetList(form)
	c.JSON(http.StatusOK, appresp.Reps(ret, err))
}

func (hdl *userVerificationHdl) Get(c *gin.Context) {
	info := new(model.UserVerification)
	if err := c.ShouldBind(info); err != nil {
		c.JSON(http.StatusBadRequest, appresp.Err(err))
		return
	}

	ret, err := user_verification_admin.AdminSrv.Get(info.Id)
	c.JSON(http.StatusOK, appresp.Reps(ret, err))
}
...
func (hdl *userVerificationHdl) Add(c *gin.Context) {
	...
	err := iuser_verification.Srv.Insert(info)
...
func (hdl *userVerificationHdl) Update(c *gin.Context) {
	...
	_, err := iuser_verification.Srv.Update(info)
	c.JSON(http.StatusOK, appresp.Reps(info, err))
}
...
func (hdl *userVerificationHdl) Delete(c *gin.Context) {
	...
	err := iuser_verification.Srv.Delete(info.Id)
...
func (hdl *userVerificationHdl) SetInfo(c *gin.Context) {
	...
	affected, err := iuser_verification.Srv.SetInfo(info)
